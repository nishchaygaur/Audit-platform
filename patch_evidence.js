...
let content2 = fs.readFileSync('src/app/audits/[id]/evidence/page.tsx', 'utf8');
content2 = content2.replace(
  ...
  require('react').useEffect(() => {
    if (workspaceId && params.id) {
      require('@/actions/evidence').getEvidences(workspaceId, params.id as string).then((res: any) => {
        if (res.success && res.data) {
          setEvidence(res.data.map((e: any) => ({...e, evidenceId: e.reference, uploadedBy: e.uploaded_by, uploaded: e.date})));
        }
        setLoading(false);
...
fs.writeFileSync('src/app/audits/[id]/evidence/page.tsx', content2);
console.log("Patched evidence.");
